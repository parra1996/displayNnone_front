...
export class PedidosComponent implements OnInit {
  ...
  private calculateOrder(order: PedidoInterface) {
    ...
    switch (true) {
      case weight <= 0.1:
        return (
          (this.orderForm.value.price = weight * 5),
          (this.orderForm.value.order_type = 'paquete ultra ligero')
        );
      case weight >= 0.1 && weight <= 0.3:
        return (
          (this.orderForm.value.price = weight * 5 + 1),
          (this.orderForm.value.order_type = 'paquete ligero')
        );
      case weight >= 0.3 && weight <= 5:
        return (
          (this.orderForm.value.price = weight * 10),
          (this.orderForm.value.order_type = 'paquete estandar')
        );
      case weight >= 5 && weight <= 10:
        return (
          (this.orderForm.value.price = weight * 5 + weight + 75),
          (this.orderForm.value.order_type = 'paquete pesado')
        );
      case weight >= 10:
        return (
          (this.orderForm.value.price = (weight - 10) * 75 + 130 + weight),
          (this.orderForm.value.order_type = 'paquete pesado')
        );
    }
  }
